...
class TestCommandTopupSoc(unittest.TestCase):
    def setUp(self):
        self._tmp = tempfile.TemporaryDirectory()
        self._state_dir = Path(self._tmp.name)
        import growatt_guard.state as _state
    ...
    def test_already_on_utility_observed_returns_early(self):
        from growatt_guard.modes import command_topup_soc
        ...
        with patch("growatt_guard.modes.load_context", return_value=(MagicMock(), MagicMock(), status)):
            ...
            sys.stdout = captured
            try:
                rc = command_topup_soc(cfg, 40.0)
            finally:
                sys.stdout = sys.__stdout__
        self.assertEqual(rc, 0)
        self.assertIn("Already on Utility", captured.getvalue())
        self.assertIn("No topup started", captured.getvalue())

    def test_already_above_target_skips(self):
        from growatt_guard.modes import command_topup_soc
        cfg = _make_topup_config()
        status = _make_status_mock(soc=45.0, on_utility=False)
        with patch("growatt_guard.modes.load_context", return_value=(MagicMock(), MagicMock(), status)):
            import io, sys
            captured = io.StringIO()
            sys.stdout = captured
            try:
                ...
                sys.stdout = sys.__stdout__
        self.assertEqual(rc, 0)
        self.assertIn("already at or above target", captured.getvalue())

    def test_starts_topup_dry_run(self):
        from growatt_guard.modes import command_topup_soc
        from growatt_guard.state import read_utility_hold_state
        cfg = _make_topup_config()
        status = _make_status_mock(soc=32.0, on_utility=False)
        with (
            ...
            try:
                ...
                sys.stdout = sys.__stdout__
        self.assertEqual(rc, 0)
        self.assertIn("Topup started", captured.getvalue())
        # dry_run=True: no actual state should be written
        self.assertIsNone(read_utility_hold_state())

    def test_raises_without_capacity_config(self):
        from growatt_guard.modes import command_topup_soc
        from growatt_guard.exceptions import GrowattGuardError
        cfg = make_config()  # no battery_capacity_wh
        status = _make_status_mock(soc=32.0, on_utility=False)
        with self.assertRaises(GrowattGuardError):
            with patch("growatt_guard.modes.load_context", return_value=(MagicMock(), MagicMock(), status)):
                command_topup_soc(cfg, 40.0)


# ---------------------------------------------------------------------------
# command_adopt_utility tests
# ---------------------------------------------------------------------------
